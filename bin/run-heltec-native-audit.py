...
with (report / "native.log").open("w") as log:
    with subprocess.Popen(
        ...
        for line in process.stdout:
            log.write(line)
            if line.startswith("Processing ") or re.search(
                r"\[(PASSED|FAILED|ERRORED)\]", line
            ):
...
for name in suites:
    ...
    if not cases or any(
        c.find(x) is not None for c in cases for x in ("failure", "error", "skipped")
    ):
