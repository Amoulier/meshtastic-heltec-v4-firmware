...
// Used only for a replacement, never for a rejected candidate.
...
// A candidate rejected before mutation never replaces the current identity.
static const char LOW_ENTROPY_REJECT_WARNING[] =
    "That key is a known pre-2.8 low-entropy key and can't be restored. "
    "The current identity and node number were preserved.";
#endif
