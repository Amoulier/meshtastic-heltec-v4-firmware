...
def build_run(name, cpp, extra=()):
    with tempfile.TemporaryDirectory(prefix='heltec-audit-') as d:
        ...
        subprocess.run(['g++','-std=c++17','-O1','-g','-Wall','-Wextra','-Werror',
                        ...
                        '-fno-pie','-no-pie','-pthread',f'-I{ROOT / "src"}',*extra,str(source),'-o',str(exe)],check=True)
...
'''
sx_methods = '\n\n'.join('template <typename T> '+body('src/mesh/SX126xInterface.cpp',sig) for sig in [
...
for m in re.finditer(r'radioHardwareParked\.store\(false, std::memory_order_release\)',sx):
    assert 'radioSleepConfirmed.store(false' in sx[max(0,m.start()-100):m.start()]
for path in ['src/graphics/Screen.cpp','src/graphics/Screen.h','src/graphics/draw/MessageRenderer.cpp']:
    text=(ROOT/path).read_text()
    assert 'textMessageFrameShown' not in text and 'isTextMessageFrameShown' not in text
assert 'std::atomic<bool> displayDisabled' in (ROOT/'src/graphics/Screen.h').read_text()
...
assert 'current identity and node number were preserved' in (ROOT/'src/mesh/NodeDB.h').read_text()
assert 'if (!deliverLocal(p, src))' in body('src/mesh/Router.cpp','ErrorCode Router::sendLocal(')
